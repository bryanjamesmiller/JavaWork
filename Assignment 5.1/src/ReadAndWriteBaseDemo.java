...
import java.util.*;
//import java.io.*;      // Needed to allow the use of 
...
class ReadAndWriteBaseDemo
{
    ...
    static int readBase (int base) 
    {
        ...
        keyboard.useDelimiter(""); //reads one character at a time, 
        //so 36 is read as 3 and then 6 separately

        do                                      // Ignore white space and
        {
            ch = keyboard.next().charAt(0); 	// non-base chars
        }
        // ch - '0' converts character values to integer values(?) i think.
