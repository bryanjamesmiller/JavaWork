...
public class Heap<T extends Comparable<T>> {
	...
	private void siftDown(int i) {
		// the heap is stored in an array So i is an index

		int parent = i;
		T toSift = contents[parent]; // The parent value (whatever it is) is store to toShift

		int child = 2 * parent + 1;  //simply how you find a parent's left child array index

		//Compare left and right child to find the bigger of the 2 (and make sure the array size is big enough)
		if (child < numItems - 1  && contents[child].compareTo(contents[child + 1]) < 0)
			child = child + 1; //if the right child is bigger than the left, select it for swapping instead of the left child by incrementing the array index

		//Compare that bigger child with the parent
		if (toSift.compareTo(contents[child]) >= 0)
			return;  //if the parent is bigger than the bigger child, then the parent is in the right place and our base case is reached here

		//Otherwise swap parent with child and recursive call with the new position.

		contents[parent] = contents[child];
		contents[child] = toSift;

		//the parent has now become the child in reality, so we call this function on the parent (again)
		siftDown(parent);
