 ...
 *  a chess piece (bishops, knights and kings) 
...
class Chessboard 
{
	...
	public static void main (String [] args ) 
	{
		...
		if(answer.length()==1)
			if(answer.charAt(0)=='k' || answer.charAt(0)=='K')
			{
				System.out.println("Please clarify, knight or king?");
				System.out.print ("Would you like to play with a Bishop, Knight or King? ");
				answer = keyboard.nextLine ();
			}
		if (answer.charAt(0) == 'b' || answer.charAt(0) == 'B') 
			p = new Bishop();
		else if (answer.equalsIgnoreCase("King"))
			p= new King();
		else 
			p = new Knight();
